...
function ProjectCard({project, index, isRevealed}: {project: Project; index: number; isRevealed?: boolean}) {
  ...
  return (
    <motion.a
      ...
      transition={isRevealed
        ? {duration: 0.3, delay: (index - featuredProjects.length) * 0.06, ease: EASE, ...LAYOUT_TRANSITION}
        : {duration: 0.5, delay: index * 0.1, ease: EASE, ...LAYOUT_TRANSITION}
      }
...
function MoreProjectsCard({className}: {className?: string}) {
  const t = useTranslations('projects');
  ...
  return (
    <motion.div
      layout
      layoutId="more-projects"
      ref={ref}
      initial={{opacity: 0, y: 20}}
      whileInView={{opacity: 1, y: 0}}
      viewport={{once: true, margin: '-80px'}}
      transition={{duration: 0.5, ease: EASE, ...LAYOUT_TRANSITION}}
...
export function Projects() {
  ...
  const remainingAfterProjects = (COLS - (visibleCount % COLS)) % COLS;
  const moreColSpan =
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <LayoutGroup>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <AnimatePresence>
              {showAll && hiddenProjects.map((project, i) => (
                <ProjectCard
                  key={project.title}
                  project={project}
            ...
            <MoreProjectsCard className={moreColSpan} />
        ...
        {hiddenProjects.length > 0 && (
          <motion.div
            ...
            transition={{duration: 0.4, delay: (visibleCount + 2) * 0.1, ease: EASE}}
